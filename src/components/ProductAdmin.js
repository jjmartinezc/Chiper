...
export default class ProductAdmin extends Component {
  ...
  handleUpdateProduct = async (id, name) => {
    ...
    try {
      ...
      await axios.patch(`${config.api.invokeUrl}/productos/${id}`, params);
  ...
  handleDeleteProduct = async (id, event) => {
    ...
    try {
      await axios.delete(`${config.api.invokeUrl}/productos/${id}`);
      const updatedProducts = [...this.state.products].filter(product => product.id !== id);
      this.setState({products: updatedProducts});
    }catch (err) {
      ...
      <Fragment>
        <section className="section">
          <div className="container">
            <h1>Productos tenderos</h1>
            <p className="subtitle is-5"> Agrega o quita los productos usando el formato de abajo:</p>
            <br />
              ...
              <div className="column is-one-third">
                <form onSubmit={event => this.handleAddProduct(this.state.newproduct.id, event)}>
                  <div className="field has-addons">
                    ...
                    <div className="control">
                      <button type="submit" className="button is-danger is-medium">
                        Agregar producto
                      </button>
                    </div>
                  </div>
